...
namespace
{
	...
	bool test_vec(float* a, float* b)
	{
		...
		c[11] = CodeEngine::math::normalize(*reinterpret_cast<T2*>(a));
...
{
	{
		...
		for (int i = 0; i < 100; i++)
		{
			...
			if (!test_vec<glm::vec2, CodeEngine::math::float2>(a, b)) return -1;

			if (!test_vec<glm::vec3, CodeEngine::math::float3>(a, b)) return -1;

			if (!test_vec<glm::vec4, CodeEngine::math::float4>(a, b)) return -1;

			if (!check_vec_equality(glm::cross(*reinterpret_cast<glm::vec3*>(a), *reinterpret_cast<glm::vec3*>(b)),
				CodeEngine::math::cross(*reinterpret_cast<CodeEngine::math::float3*>(a), *reinterpret_cast<CodeEngine::math::float3*>(b))))
			{
				return __LINE__;
			}
		}
		...
		for (int i = 0; i < 100; i++)
		{
			...
			CodeEngine::math::quat q2 = CodeEngine::math::euler(pitch, yaw, roll);
			...
			CodeEngine::math::float3 r2 = q2 * (*reinterpret_cast<CodeEngine::math::float3*>(vec));
			...
			CodeEngine::math::float4x4 m2 = CodeEngine::math::transform(
				CodeEngine::math::float3{ tx, ty, tz },
				q2,
				CodeEngine::math::float3{ sx, sy, sz }
			);
			...
			CodeEngine::math::float4 t = m2 * *reinterpret_cast<CodeEngine::math::float4*>(vec);
			r2 = CodeEngine::math::float3{ t.x, t.y, t.z };
		...
		for (uint32_t i = 0; i < 100; i++)
		{
			...
			CodeEngine::math::quat q2 = CodeEngine::math::angleAxis(
				theta,
				CodeEngine::math::normalize(*reinterpret_cast<CodeEngine::math::float3*>(axii))
			);
		...
		for (uint32_t i = 0; i < 100; i++)
		{
			...
			CodeEngine::math::quat a2 = CodeEngine::math::euler(pitch1, yaw1, roll1);
			CodeEngine::math::quat b2 = CodeEngine::math::euler(pitch2, yaw2, roll2);

			glm::quat c1 = glm::slerp(a1, b1, t);
			CodeEngine::math::quat c2 = CodeEngine::math::slerp(a2, b2, t);
			...
			c2 = CodeEngine::math::slerp(a2, a2, 0.5f);
