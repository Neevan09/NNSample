...
bool Camera::BumperCheck(CodeEngine::math::float3 pos)
{
	RayTestResult hit = {};
	CodeEngine::math::float3 back = -1.0f * tCamera->GetForwardVector();
	CodeEngine::math::float3 right = tCamera->GetRightVector();
	CodeEngine::math::float3 left = -1.0f * tCamera->GetRightVector();
...
//void Camera::AdjustCameraPosition(RayTestResult hit, CodeEngine::math::float3 toTarget, float dT)
//{
//	//float dist = (transform.position - target.transform.position).magnitude;
//	float dist = math::length( (camPos - camTarget) );
//	//float distance = ((hit.distance * 100) / bumperMaxDistance) * maxDistance;
...
// Set Skybox
//void Camera::SetSkybox(CodeEngine::Material* skyBox)
void Camera::SetSkybox(CodeEngine::TextureHandle skyBox)
{
