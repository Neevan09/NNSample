...
//using CodeEngine::math::float2;
//using CodeEngine::math::float3;
//using CodeEngine::math::float4;
//using CodeEngine::math::quat;
//using CodeEngine::math::float4x4;
//using CodeEngine::math::int4;
//using CodeEngine::math::uint4;

typedef CodeEngine::model::ModelMesh Mesh;
typedef CodeEngine::model::ModelBone Bone;
typedef CodeEngine::model::ModelAnimation Animation;
...
using namespace CodeEngine::compression;
...
uint16_t loadBoneHierarchy(const aiNode* node, BoneTree& bones, BoneTable& table, FbxNodeTable& fbxNodeTable, uint16_t parentId = UINT16_MAX, uint16_t lastSibling = UINT16_MAX)
{
	...
	for (int i = 0; i < bones.size(); i++) {
		CodeEngine::math::float3 t, s;
		CodeEngine::math::quat q;

		CodeEngine::math::decompose(bones[i].transformMatrix, t, q, s);
		bones[i].transform = std::move(CodeEngine::Transform(t, q, s));
	}
...
struct ModelFile
{
	...
	CodeEngine::model::ModelHeader		header;
	...
	int Init(const char* filename, bool withAnim = true, bool ignoreMesh = false)
	{
		...
		header = {};
		header.Magic = CodeEngine::model::kModelFileMagic;
		header.Version = CodeEngine::model::kModelFileVersion;
		...
		if (header.NumTexcoordChannels > CodeEngine::model::kModelFileMaxTexcoordChannels)
		{
			header.NumTexcoordChannels = CodeEngine::model::kModelFileMaxTexcoordChannels;
		}
		...
		{
			const aiMesh* mesh = scene->mMeshes[i];
			CodeEngine::model::ModelMesh& meshInfo = meshes[i];
	...
	int RetargetAnimation(ModelFile& other, uint32_t animId, const char* name = nullptr)
	{
		...
		for (uint32_t i = 0; i < srcAnim.numFrames; i++)
		{
			...
			if (type == CodeEngine::model::kChannelRotation)
			{
				quat q;
			...
			else if (type == CodeEngine::model::kChannelTranslation)
			{
			...
			else if (type == CodeEngine::model::kChannelScale)
			{
				//
			}
