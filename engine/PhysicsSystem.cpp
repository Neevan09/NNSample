...
namespace
{
	TF_INLINE btVector3 btVec3(const CodeEngine::math::float3& v)
	{
		return btVector3(v.x, v.y, v.z);
	}

	TF_INLINE btQuaternion btQuat(const CodeEngine::math::quat& q)
	{
		return btQuaternion(q.x, q.y, q.z, q.w);
	}

	TF_INLINE CodeEngine::math::float3 tfVec3(const btVector3& v)
	{
		return CodeEngine::math::float3(v.x(), v.y(), v.z());
	}

	TF_INLINE CodeEngine::math::quat tfQuat(const btQuaternion& q)
	{
		return CodeEngine::math::quat(q.w(), q.x(), q.y(), q.z());
	}

	void AddToBulletTriangleMesh(btTriangleMesh* mesh, CodeEngine::Model* model)
	{
		...
		for (uint32_t iMesh = 0; iMesh < numSubMesh; iMesh++)
		{
			...
			for (uint32_t iTriangle = 0; iTriangle < numTriangles; iTriangle++)
			{
				...
				auto& v1 = *reinterpret_cast<const CodeEngine::math::float3*>(vertices + stride * i1);
				auto& v2 = *reinterpret_cast<const CodeEngine::math::float3*>(vertices + stride * i2);
				auto& v3 = *reinterpret_cast<const CodeEngine::math::float3*>(vertices + stride * i3);
	...
	{
	public:
		TofuMeshAdaptor(CodeEngine::Model* model) : model(model) {}
